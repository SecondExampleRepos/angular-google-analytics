...
const useAnalytics = (config: AnalyticsConfig) => {
  ...
  const generateCommandName = (commandName: string, config: any): string =>
    typeof config === 'string'
      ? `${config}.${commandName}`
      : isPropertyDefined('name', config)
      ? `${config.name}.${commandName}`
      : commandName;

  const getUrl = (): string => {
    const url = config.trackUrlParams ? location.search : location.pathname;
  ...
  const _gaq = (...args: any[]) => {
    if (config.offlineMode) {
      setOfflineQueue((prevQueue) => [...prevQueue, [_gaq, args]]);
      return;
    }
    if (!window._gaq) {
      window._gaq = [];
  ...
  const _ga = (...args: any[]) => {
    if (config.offlineMode) {
      setOfflineQueue((prevQueue) => [...prevQueue, [_ga, args]]);
      return;
    }
  ...
  const registerScriptTags = () => {
    ...
    if (config.disableAnalytics) {
      config.accounts?.forEach((trackerObj) => {
        ...
        (window as any)[`ga-disable-${trackerObj.tracker}`] = true;
      });
    }
  ...
  const registerTrackers = () => {
    ...
    if (config.universalAnalytics) {
      config.accounts.forEach((trackerObj) => {
        trackerObj.crossDomainLinker = isPropertyDefined('crossDomainLinker', trackerObj)
          ? trackerObj.crossDomainLinker
          : config.crossDomainLinker;
        trackerObj.crossLinkDomains = isPropertyDefined('crossLinkDomains', trackerObj)
          ? trackerObj.crossLinkDomains
          : config.crossLinkDomains;
        trackerObj.displayFeatures = isPropertyDefined('displayFeatures', trackerObj)
          ? trackerObj.displayFeatures
          : config.displayFeatures;
        trackerObj.enhancedLinkAttribution = isPropertyDefined('enhancedLinkAttribution', trackerObj)
          ? trackerObj.enhancedLinkAttribution
          : config.enhancedLinkAttribution;
        trackerObj.set = isPropertyDefined('set', trackerObj) ? trackerObj.set : {};
        trackerObj.trackEcommerce = isPropertyDefined('trackEcommerce', trackerObj)
          ? trackerObj.trackEcommerce
          : config.ecommerce;
        trackerObj.trackEvent = isPropertyDefined('trackEvent', trackerObj) ? trackerObj.trackEvent : false;

        const fields = isPropertyDefined('fields', trackerObj) ? trackerObj.fields : {};
  ...
  const _addTrans = (
    ...
    _analyticsJs(() => {
      if (_ecommerceEnabled(true, 'addTrans')) {
        ...
        _gaMultipleTrackers(includeFn, 'ecommerce:addTransaction', {
          id: transactionId,
          affiliation,
          revenue: total,
          tax,
  ...
  const _addItem = (
    ...
    _analyticsJs(() => {
      if (_ecommerceEnabled(true, 'addItem')) {
        ...
        _gaMultipleTrackers(includeFn, 'ecommerce:addItem', {
          id: transactionId,
          name,
          sku,
          category,
  ...
  const _addImpression = (
    ...
    _analyticsJs(() => {
      if (_enhancedEcommerceEnabled(true, 'addImpression')) {
        const includeFn = (trackerObj: Tracker) => isPropertySetTo('trackEcommerce', trackerObj, true);
        _gaMultipleTrackers(includeFn, 'ec:addImpression', {
          id,
          name,
          category,
          brand,
  ...
  const _addPromo = (
    ...
    _analyticsJs(() => {
      if (_enhancedEcommerceEnabled(true, 'addPromo')) {
        const includeFn = (trackerObj: Tracker) => isPropertySetTo('trackEcommerce', trackerObj, true);
        _gaMultipleTrackers(includeFn, 'ec:addPromo', {
          id: productId,
          name,
          creative,
          position,
